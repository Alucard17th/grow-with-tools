  ...
  <form method="POST" action="{{ route('posts.update', $post) }}" enctype="multipart/form-data" class="row g-3">
    ...
    <div class="col-md-4">
      ...
      <input name="product" class="form-control @error('product') is-invalid @enderror"
             value="{{ old('product', $post->product) }}" placeholder="doctor_native or habit_tracker" {{ $ro }} required>
    ...
    <div class="col-12">
      ...
      <textarea name="caption" rows="4" class="form-control @error('caption') is-invalid @enderror" {{ $ro }}>{{ old('caption', $post->caption) }}</textarea>
      @error('caption')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>

    <div class="col-md-4">
      <label class="form-label">Video source</label>
      <select id="video_source" name="video_source" class="form-select @error('video_source') is-invalid @enderror" {{ $dis }}>
    ...
    <div class="col-md-4">
      ...
      <select name="privacy" class="form-select @error('privacy') is-invalid @enderror" {{ $dis }}>
        @foreach (['PUBLIC_TO_EVERYONE','MUTUAL_FOLLOW_FRIENDS','FOLLOWER_OF_CREATOR','SELF_ONLY'] as $opt)
          <option value="{{ $opt }}" {{ old('privacy', $post->privacy)===$opt ? 'selected' : '' }}>{{ $opt }}</option>
        @endforeach
      </select>
    ...
    <div class="col-md-4 d-flex align-items-center gap-3 flex-wrap">
      ...
      @foreach ($checks as $name => $label)
        <div class="form-check">
          <input class="form-check-input @error($name) is-invalid @enderror" type="checkbox" name="{{ $name }}" id="{{ $name }}" value="1"
                 {{ old($name, $post->$name) ? 'checked' : '' }} {{ $dis }}>
